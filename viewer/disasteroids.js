...
window.addEventListener("load", function() {
	...
	Q.VectorSprite.extend("Ship", {
		init: function(p, color) {
			...
			this.activationObject = new Q.Sprite({
				x: Q.width / 2,
				y: Q.height / 2,
				w: 100,
				h: 100
		...
		up: function(amount) {
			this.p.acceleration=amount;
		},
		lr: function(amount) {
			this.p.omegaDelta=amount;
		},

		draw: function(ctx) {
			if (this.p.activated) {
				this._super(ctx, this.color);
			}
		},
	...
	Q.scene("game", function(stage) {
		viewer.players.forEach(function(player) {
			stage.insert(player);
		});
